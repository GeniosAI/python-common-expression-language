...
info_text = evaluate(
    "format_user_info(employee.name, employee.age, employee.department)", demo_context
)
...
error_context.add_variable("users_db", {"user123": {"name": "Alice", "status": "active"}})
```
...
class PolicyContext:
    ...
    def add_request_info(self, method, path, ip_address):
        ...
        self.context.add_variable(
            "request",
            {"method": method, "path": path, "ip": ip_address, "time": datetime.now().isoformat()},
        )
        return self
